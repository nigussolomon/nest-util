/**
 * Generic English defaults for every error key. Apps deep-merge their JSON
 * message config over this map so they only supply languages/overrides they care
 * about. This is the canonical "library defaults" source (mirrored by `en.json`).
 */
export const defaultMessages: Record<string, string> = {
  // ---- CRUD ----
  CRUD_RESOURCE_NOT_FOUND: 'The requested resource was not found',
  CRUD_RELATION_NOT_FOUND: 'The referenced related record does not exist',
  CRUD_FORBIDDEN: 'You are not allowed to perform this action',
  CRUD_FIND_MINE_NOT_CONFIGURED: 'This resource does not support the requested scope',
  CRUD_INVALID_STATUS: 'The provided status is not allowed',
  CRUD_STATUS_NOT_CONFIGURED: 'The status pipeline is not configured for this resource',
  CRUD_STATUS_TRANSITION_INVALID: 'The requested status change is not allowed',
  CRUD_STATUS_TRANSITION_FORBIDDEN: 'You do not have permission to change this status',
  CRUD_INVALID_FILTER: 'The provided filter is not allowed',
  CRUD_APPROVAL_NOT_CONFIGURED: 'Approvals are not enabled for this resource',
  CRUD_APPROVAL_FORBIDDEN: 'You do not have permission to perform this approval action',
  CRUD_APPROVAL_INVALID_TRANSITION: 'The requested approval change is not allowed',
  CRUD_APPROVAL_RESUBMIT_NOT_SATISFIED: 'Requested modifications have not been applied yet',

  // ---- Auth ----
  AUTH_INVALID_CREDENTIALS: 'Invalid credentials',
  AUTH_USER_ALREADY_EXISTS: 'An account with this identifier already exists',
  AUTH_USER_NOT_FOUND: 'The requested account was not found',
  AUTH_UNAUTHORIZED: 'Authentication is required',
  AUTH_PERMISSION_DENIED: 'You do not have permission to perform this action',
  AUTH_TOKEN_INVALID: 'The provided token is invalid',
  AUTH_TOKEN_EXPIRED: 'The provided token has expired',
  AUTH_TOKEN_REUSED: 'The provided token has been revoked',
  AUTH_ACCOUNT_NOT_VERIFIED: 'This account is not verified',
  AUTH_REFRESH_TOKEN_REQUIRED: 'A refresh token is required',
  AUTH_LOGOUT_FAILED: 'Unable to sign out at this time',
  AUTH_CURRENT_PASSWORD_INCORRECT: 'The current password is incorrect',
  AUTH_PASSWORD_NOT_SET: 'This account does not have a password set',
  AUTH_NO_UPDATABLE_FIELDS: 'No updatable fields were provided',
  AUTH_REGISTER_FIELD_NOT_ALLOWED: 'One or more provided fields are not allowed',
  AUTH_OTP_DELIVERY_FAILED: 'Unable to send the verification code',
  AUTH_OTP_INVALID: 'The verification code is invalid',
  AUTH_OTP_EXPIRED: 'The verification code has expired',
  AUTH_OTP_RATE_LIMITED: 'Too many attempts. Please try again later',
  AUTH_PASSWORD_RESET_TOKEN_INVALID: 'The password reset token is invalid',
  AUTH_PASSWORD_RESET_TOKEN_EXPIRED: 'The password reset token has expired',
  AUTH_PASSWORD_RESET_TOKEN_REQUIRED: 'A password reset token is required',
  AUTH_PASSWORD_RESET_DELIVERY_FAILED: 'Unable to send the password reset link',
  AUTH_ROLE_NAME_REQUIRED: 'A role name is required',
  AUTH_ROLE_ALREADY_EXISTS: 'A role with this name already exists',
  AUTH_ROLE_NOT_FOUND: 'The requested role was not found',
  AUTH_ROLE_PERMISSIONS_REQUIRED: 'Permissions are required',
  AUTH_ROLE_ASSIGNMENT_FAILED: 'Unable to assign the role',
  AUTH_ROUTE_DISABLED: 'This route is disabled',
  AUTH_API_KEY_MISSING: 'An API key is required',
  AUTH_API_KEY_INVALID: 'The API key is invalid',
  AUTH_API_KEY_EXPIRED: 'The API key has expired',
  AUTH_API_KEY_REVOKED: 'The API key has been revoked',
  AUTH_API_KEY_NOT_FOUND: 'The requested API key was not found',
  AUTH_API_KEY_ROLE_ASSIGN_FAILED: 'Unable to assign the role to the API key',
  AUTH_API_KEY_ROLE_NOT_FOUND: 'One or more of the requested roles was not found',
  AUTH_PASSWORD_RESET_NEW_REQUIRED: 'A new password is required',
  AUTH_ONBOARDING_NOT_ENABLED: 'Onboarding is not enabled',
  AUTH_ONBOARDING_TOKEN_REQUIRED: 'An onboarding token is required',
  AUTH_ONBOARDING_TOKEN_INVALID: 'The onboarding token is invalid',
  AUTH_ONBOARDING_TOKEN_USED: 'The onboarding token has already been used',

  // ---- Auth: OTP / verification / password-reset plumbing ----
  AUTH_OTP_NOT_ENABLED: 'OTP authentication is not enabled',
  AUTH_OTP_CODE_LENGTH_INVALID: 'The OTP code length is invalid',
  AUTH_VERIFICATION_NOT_ENABLED: 'Account verification is not enabled',
  AUTH_VERIFICATION_CODE_REQUIRED: 'A verification code is required',
  AUTH_VERIFICATION_CODE_INVALID: 'The verification code is invalid',
  AUTH_VERIFICATION_CODE_EXPIRED: 'The verification code has expired',
  AUTH_VERIFICATION_RATE_LIMITED: 'Too many verification attempts. Please try again later',
  AUTH_VERIFICATION_DELIVERY_FAILED: 'Unable to send the verification code',
  AUTH_PASSWORD_RESET_NOT_ENABLED: 'Password reset is not enabled',
  AUTH_PASSWORD_CHANGE_FAILED: 'Unable to change the password',
  AUTH_SESSION_UPDATE_FAILED: 'Unable to update the session',
  AUTH_OTP_SESSION_UPDATE_FAILED: 'Unable to update the OTP session',
  AUTH_RESET_STATE_FAILED: 'Unable to update the password reset state',
  AUTH_VERIFICATION_STATE_FAILED: 'Unable to update the verification state',
  AUTH_ONBOARDING_DELIVERY_FAILED: 'Unable to send the onboarding code',
  AUTH_ONBOARDING_CODE_INVALID: 'The onboarding code is invalid',
  AUTH_ONBOARDING_RATE_LIMITED: 'Too many onboarding attempts. Please try again later',
  AUTH_ONBOARDING_CODE_REQUIRED: 'An onboarding code is required',
  AUTH_ONBOARDING_ATTEMPTS_EXCEEDED: 'Maximum onboarding attempts reached',
  AUTH_IDENTIFIER_MISMATCH: 'The provided identifier does not match',

  // ---- Notify / Payment / File ----
  NOTIFY_SOCKET_DISABLED: 'Realtime notifications are disabled',
  NOTIFY_FCM_NOT_CONFIGURED: 'Push notifications are not configured',
  NOTIFY_SMTP_NOT_CONFIGURED: 'Email notifications are not configured',
  NOTIFY_DEVICE_TOKEN_INVALID: 'The device token is invalid',
  NOTIFY_PUSH_FAILED: 'Unable to deliver the push notification',
  NOTIFY_EMAIL_DELIVERY_FAILED: 'Unable to send the email',
  PAYMENT_PROVIDER_NOT_CONFIGURED: 'Payments are not configured',
  PAYMENT_PAYMENT_NOT_FOUND: 'The requested payment was not found',
  PAYMENT_SUBSCRIPTION_NO_PROVIDER: 'The subscription has no provider reference',
  PAYMENT_RECONCILIATION_DISABLED: 'Reconciliation is disabled',
  PAYMENT_REFUND_NOT_FOUND: 'The requested refund was not found',
  PAYMENT_CHECKOUT_FAILED: 'Unable to start the checkout session',
  PAYMENT_SUBSCRIPTION_NOT_FOUND: 'The requested subscription was not found',
  PAYMENT_REFUND_FAILED: 'Unable to process the refund',
  PAYMENT_WEBHOOK_INVALID: 'The webhook payload is invalid',
  FILE_NOT_FOUND: 'The requested file was not found',
  FILE_TOO_LARGE: 'The uploaded file is too large',
  FILE_INVALID_TYPE: 'The uploaded file type is not allowed',
  FILE_UPLOAD_FAILED: 'Unable to upload the file',

  // ---- Database / generic ----
  NOT_FOUND: 'The requested resource was not found',
  CONFLICT: 'The request conflicts with the current state',
  DB_DUPLICATE_ENTRY: 'A record with this value already exists',
  DB_QUERY_FAILED: 'A database error occurred',
  VALIDATION_FAILED: 'Some of the provided data is invalid',
  INTERNAL_ERROR: 'An unexpected error occurred. Please try again later',
  UNKNOWN_ERROR: 'An unexpected error occurred',

  // ---- Success verbs ----
  MSG_CREATED: 'created',
  MSG_UPDATED: 'updated',
  MSG_DELETED: 'deleted',
  MSG_FETCHED: 'fetched',
  MSG_REQUEST_SUCCESS: 'Request successful',
  SUCCESS_FORMAT: '{entity} {action} successfully',
};

/** Convenience accessor mirroring `defaultMessages`. */
export function getDefaultMessage(code: string): string | undefined {
  return defaultMessages[code];
}
